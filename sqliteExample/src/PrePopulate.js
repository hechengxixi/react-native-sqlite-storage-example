...
const database_name = "testDB.sqlite3";
let db;
...
export default class SQLiteDemo extends Component {
  ...
  displayDatabase = ()=>{
    this.updateProgress("Displaying database");
    SQLite.echoTest().then(() => {
      this.updateProgress("Integrity check passed ...");
      this.updateProgress("Opening database ...");
      SQLite.openDatabase({name:database_name,createFromLocation:1}).then((DB) => {
        db = DB;
        this.updateProgress("Database OPEN");
        DB.transaction(this.queryDepartments)
        .then((result) => {
            console.log("Transaction is now finished");
            this.updateProgress("Processing completed");
            this.closeDatabase()
  ...
  queryDepartments = (tx)=>{
    ...
    .then(([tx,results])=>{
      this.updateProgress("SELECT Departments success");
      console.log(results);
      const output = [];
  ...
  render(){
    return (<View style={styles.mainContainer}>
      <View style={styles.toolbar}>
        ...
        <Text style={styles.toolbarButton} onPress={this.displayDatabase}>
          display DB
        </Text>
      </View>
      <View>
        <Text>{JSON.stringify(this.state.progress)}</Text>
      </View>
       <View>
